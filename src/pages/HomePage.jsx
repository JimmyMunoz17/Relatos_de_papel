  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
            ...
            <SwiperSlide key={b.id}>
              <div className="bg-white p-10 flex flex-col md:flex-row items-center gap-10">
                ...
                <div className="flex-1">
                  <span className="text-blue-600 font-bold text-xs uppercase tracking-widest">{b.category}</span>
                  <h3 className="text-4xl font-bold text-gray-900 mt-2">{b.title}</h3>
                  <p className="text-gray-600 mt-4 text-lg line-clamp-3">{b.description}</p>
                  <div className="mt-8 flex items-center gap-6">
                    <span className="text-3xl font-bold text-gray-900">${b.price}</span>
                    <button 
                      onClick={() => addItem(b)}
                    ...
                    <Link 
                      to={`/book/${b.id}`}
                      className="text-gray-500 font-semibold hover:text-blue-600 transition"
                    >
                      Ver detalles
                    </Link>
                  </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {books.map(b => (
          <div key={b.id} className="border p-4 rounded-xl flex flex-col bg-white hover:shadow-lg transition-all group">
